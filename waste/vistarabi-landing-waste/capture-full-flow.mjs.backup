...
async function capture() {
    ...
    try {
        ...
        await page.screenshot({ path: path.join(artifactsDir, 'landing_page.png') });

        // 2. Auth Page (Sign In)
        console.log('Capturing Sign In Page...');
        await page.goto('http://localhost:3005/login', { waitUntil: 'domcontentloaded', timeout: 60000 });
        ...
        await page.goto(`http://localhost:3005/app/projects`);
        ...
        // Module 7 & 8: Strategy / Forecasting
        console.log('Capturing Module 7 & 8 (Forecasting & Strategy)...');
        await page.waitForFunction(() => {
            const btn = document.querySelector('button[title="Goal Strategy Engine"]');
            if (btn) btn.click();
        ...
        await page.screenshot({ path: path.join(artifactsDir, 'module_7_8_forecasting.png') });
        await page.reload({ waitUntil: 'networkidle' });
        await page.waitForTimeout(4000); // Wait for React Hydration
        ...
        await page.click('.ask-ai-send-btn');
